...
const MyVpnCardDedicated: React.FC<ReviewCardProps> = ({
  ...
  const deletevpn = async (id: string) => {
    ...
    try {
      const response = await fetch(
        `${REACT_APP_GATEWAY_URL}api/v1.0/vpn?id=${metaData.name}`,
        {
          method: "DELETE",
          headers: {
  ...
  return (
    <div className="w-full px-10">
      <div className="w-full h-full py-4 border-t border-gray-500">
        <div className="w-full flex justify-between">
          <h3 className="text-2xl leading-12 mb-2 text-white w-1/6">
          ...
          <div className="lg:flex md:flex justify-between w-1/6">
            <div>
              <button className="text-lg rounded-lg text-white">
                <a
                  href={`https://${
                    metaData.VpnEndpoint
                  }/#/auth?token=${Cookies.get("sotreus_token")}`}
                  target="_blank"
                  rel="noreferrer"
                  style={color2}
                >
          ...
          <button className="text-lg rounded-lg text-white flex w-1/6">
            <a
              href={`https://${metaData.firewallEndpoint}`}
              target="_blank"
              rel="noreferrer"
              style={color2}
            >
          ...
          <div className="lg:flex md:flex justify-between w-1/8">
            <div>
              <button
                className="text-lg rounded-lg pr-1 text-white"
      ...
      {delvpn && (
        <div
          style={{ backgroundColor: "#0E1414D9" }}
          className="flex overflow-y-auto overflow-x-hidden fixed inset-0 z-10 justify-center items-center w-full max-h-full"
          id="popupmodal"
        >
